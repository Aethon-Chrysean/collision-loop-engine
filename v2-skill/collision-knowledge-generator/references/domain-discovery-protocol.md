...
> **Why this step governs everything downstream.** The structural signature determines the *level of abstraction* at which bisociation operates. Koestler (1964) described bisociation as the perception of an event L through two habitually incompatible matrices M₁ and M₂. The event L is not a specific detail — it is a structural situation that can be *perceived* through multiple frames. If the structural signature is too specific, only one frame can perceive it (no bisociation possible). If too abstract, any frame can perceive it (no tension, no creative product). The signature must sit at the level where different domains genuinely wrestle with the same structural challenge using genuinely incompatible approaches. Finding that level is an act of judgment, not formula — but the heuristics below constrain the search space.
...
> **Why Chinese sources matter for bisociative domain discovery specifically.** Each language community develops its own retrieval pathways — its own "high-accessibility defaults" for which domains are considered relevant to which structural problems. These cross-linguistic blind spots are precisely the kind of "habitually incompatible" framing that Koestler identified as the raw material for bisociation. The bilingual search is not a completeness measure — it is itself a micro-bisociation: the Chinese research round and the English research round function as two matrices of thought about "where structural analogies live," and their intersection reveals domains that neither round would have discovered alone.
...
> **Theoretical anchor — Distinguishing structural analogy from metaphor.** Not every resemblance is a structural analogy. A metaphor maps surface features ("the brain is a computer"); a structural analogy maps *relational structure* ("the brain's synaptic pruning and a network's edge-pruning both address the same trade-off between flexibility and efficiency under resource constraints"). The distinction matters because bisociation operates on structural intersections, not on surface resemblances. If the mapping exists only at the level of metaphor, the collision will produce Failure Mode 2 (Surface-Level Analogy) from the collision-execution-guide: the output will rename an existing idea using the new domain's vocabulary without generating any structural novelty. Test every candidate mapping by asking: "Does this domain's *mechanism* address the same *structural problem*, or does it merely *look like* the home domain's problem from the outside?" Only mechanism-level mappings survive.
...
> **Theoretical anchor — Principles are the substance of the collision.** In the Geneplore model (Finke, Ward, & Smith, 1992), pre-inventive structures emerge from the interaction of specific cognitive representations — not from vague impressions. The more specific and internally coherent each principle is, the more defined the "surface" it presents to principles from other knowledge sets during the collision. Vague principles produce vague intersections; specific principles produce specific pre-inventive structures that can be explored and refined. The difference between a productive collision and a sterile one often traces back to the specificity of the principles composed in this phase.
...
> **Theoretical anchor — Synthesis is not averaging; it is itself a bisociative act.** When Chinese and English sources diverge on which domains are structurally analogous to the home domain, the divergence is data, not error. Each language community has framed the structural problem through its own "matrix of thought," and the points of divergence reveal aspects of the problem that neither community has fully articulated. Treat divergence as a signal that the structural signature (Phase 0.3) may need refinement — the structural property that produces divergence may be under-specified (too abstract to constrain the search) or may actually contain two distinct structural properties that each language community has resolved differently. In either case, the divergence improves the final output by revealing structure that convergence would have hidden.
